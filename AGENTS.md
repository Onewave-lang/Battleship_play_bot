...
- Персистентность: JSON/SQLite/PG/Redis; обязательны атомарные операции при применении ходов.
...
**Требование UX имен:** в двухпользовательском режиме инициатор после выбора режима и приглашённый после перехода по deep‑link **вводят имя** до доступа к дальнейшим действиям (меню/размещение/ходы).
...
- Механика «контур вокруг убитого» аналогична 2‑игрокам, но применяется глобально для всех.
...
- После каждого хода — **полный снимок** состояния матча и рендер **только** по последнему снимку.
- В 2‑игроках обязательный **ввод имени** у инициатора и приглашённого до доступа к действиям.
