...
def test_start_board15_join(monkeypatch):
    async def run_test():
        ...
        assert reply_text.call_args_list == [
            ...
            call('Используйте @ или ! в начале сообщения, чтобы отправить сообщение соперникам в чат игры.'),
        ]
        assert bot.send_message.call_args_list == [
            call(1, 'Соперник присоединился. Отправьте "авто" для расстановки.'),
            call(1, 'Используйте @ или ! в начале сообщения, чтобы отправить сообщение соперникам в чат игры.'),
        ]
